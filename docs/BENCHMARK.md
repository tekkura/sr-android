...
| **T4** | Android Write Complete    | `UsbSerial.send(ByteArray)` (end)                    |
| **T5** | Firmware Processing Done  | `MockRP2040.processPacket()` (after processing)      |
...
| **M4: Firmware Processing**    | $T5 - T4$   | Time spent inside `MockRP2040.processPacket()` after the firmware receives the packet. |
...
- **Mock Latency**: `VirtualRobotPort` connects to `MockRP2040`. `MockRP2040` includes a 5ms `Thread.sleep()` to simulate firmware processing time and records T5 when the simulated firmware completes processing.
...
The result snapshots below were captured before the T5/T9 timeline shift and therefore still use the older metric labels.
