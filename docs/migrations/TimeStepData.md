...
- `QRCodeData` should store a list of detection events, each containing:
    ...
    - `timestampNs`: The time of detection (provided by `ImageData.customAnalysis`, but not collected by `QRCodeDataSubscriber`. Change its signature to accommodate).
- Update `TimeStepDataBuffer.onQRCodeDetected(qrDataDecoded: String, timestamp: Long)` to append the detection to `writeData.qrCodeData`.
...
- **Backward Compatibility**: Preserve the existing flattened `getLevels()` accessor in `SoundData`. The flattened view may be derived by concatenating the per-frame `AudioFrame.levels` arrays rather than maintained as a second `ArrayList<Float>`; the frame list is the canonical source of audio samples, and avoiding duplicate mutable storage prevents the two representations from diverging. In either implementation, include only the first `sampleCount` valid samples from each callback (never unused values from a larger callback buffer), so the flattened view remains consistent with `AudioFrame.levels` and the aggregate sample counts.
